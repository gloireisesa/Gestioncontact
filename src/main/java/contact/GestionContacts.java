...
public class GestionContacts {
    ...
    public Contact rechercherParId(String id) throws SQLException {
    ...
    public List<Contact> listerContacts() throws SQLException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(selectSQL);
            ...
            while (rs.next()) {
                ...
                String email = rs.getString("email");
                // Gestion de la date d'anniversaire avec vérification de NULL
                LocalDate dateAnniv = null;
                if (rs.getDate("date_anniversaire") != null) {
                    dateAnniv = rs.getDate("date_anniversaire").toLocalDate();
                }
                String type = rs.getString("type");
    ...
    public Contact rechercherContactParId(String id) throws SQLException {
        String selectSQL = "SELECT * FROM contacts WHERE id = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(selectSQL)) {
            pstmt.setString(1, id);
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    if ("Personnel".equals(type)) {
                        String adresse = rs.getString("adresse");
                        return new ContactPersonnel(id, nom, numero, email, dateAnniv, adresse);
                    } else {
                        String entreprise = rs.getString("nom_entreprise");
                        return new ContactProfessionnel(id, nom, numero, email, dateAnniv, entreprise);
                    }
                }
            }
        }
